          ...
          function(x, w = NULL, sample = TRUE, progress = FALSE, parallel = FALSE, n = 1, cl = NULL, keep.open = FALSE){
              ...
              if (progress) {
                ...
                result <- pbapply::pbsapply(s,
                                            function(p) do.call(.covij,
                                                                list(x = subset(x, ii[p]),
                                                                     y = subset(x, jj[p]),
                                                                     w = w, sample = sample)))
              } else if (!progress) {
                result <- sapply(s, function(p) do.call(.covij,
                                                        list(x = subset(x, ii[p]),
                                                             y = subset(x, jj[p]),
                                                             w = w, sample = sample)))
              }
            ...
            if (parallel && n > 1) {
              ...
              snow::clusterExport(cl,
                                  c(".covij", "raster", "cellStats", "x", "ii", "jj", "s", "w", "canProcessInMemory", "values", "sample", "subset"),
                            envir = environment())
          ...
          function(x, y, w = NULL, sample = TRUE, progress = FALSE, parallel = FALSE, n = 1, cl = NULL, keep.open = FALSE){
            ...
            if (!parallel | n == 1) {
              if (progress) {
                ...
                result <- sapply(s, function(p) do.call(.covij, list(x = subset(x, z[p, 1]),
                                                                     y = subset(y, z[p, 2]),
                                                                     w = w, sample = sample)))
              }
            }
